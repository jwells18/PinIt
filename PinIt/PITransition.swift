...
let animationScale = w/gridWidth // screenWidth / the width of waterfall collection view's grid
class PITransition : NSObject , UIViewControllerAnimatedTransitioning{
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if presenting {
            ...
            let waterFallView = (toViewController! as! PITransitionProtocol).transitionCollectionView()!
            let pageView = (fromViewController as! PITransitionProtocol).transitionCollectionView()!
            waterFallView.superview?.layoutIfNeeded()
            let indexPath = pageView.fromPageIndexPath()
            let gridView = waterFallView.cellForItem(at: indexPath)
            let leftUpperPoint = gridView!.convert(CGPoint(x: 0, y: 64), to: toViewController!.view)
            
            let snapShot = (gridView as! PITansitionWaterfallGridViewProtocol).snapShotForTransition()
            snapShot?.transform = CGAffineTransform(scaleX: animationScale, y: animationScale)
            let pullOffsetY = (fromViewController as! PIPinDetailControllerProtocol).pageViewCellScrollViewContentOffset().y
            ...
            UIView.animate(withDuration: animationDuration, animations: {
                ...
                toView.frame = CGRect(x: 0, y: 64, width: toView.frame.size.width, height: toView.frame.size.height);
                toView.alpha = 1
            ...
            let waterFallView : UICollectionView = (fromViewController as! PITransitionProtocol).transitionCollectionView()
            let pageView : UICollectionView = (toViewController! as! PITransitionProtocol).transitionCollectionView()
            ...
            let leftUpperPoint = gridView!.convert(CGPoint(x: 0, y: 64), to: nil)
            pageView.isHidden = true
            pageView.scrollToItem(at: indexPath as IndexPath, at:.centeredHorizontally, animated: false)
            
            let offsetY : CGFloat = fromViewController!.navigationController!.isNavigationBarHidden ? 0.0 : navigationHeaderAndStatusbarHeight
            let offsetStatuBar : CGFloat = fromViewController!.navigationController!.isNavigationBarHidden ? 0.0 : statusBarHeight;
            let snapShot = (gridView as! PITansitionWaterfallGridViewProtocol).snapShotForTransition()
            containerView.addSubview(snapShot!)
